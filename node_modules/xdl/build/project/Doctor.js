...
async function _validateReactNativeVersionAsync(exp, pkg, projectRoot, sdkVersions, sdkVersion) {
  ...
  if (!isExpoForkedReactNative) {
    ...
    _internal2().ProjectUtils.clearNotification(projectRoot, 'doctor-not-using-expo-fork'); // Check react-native version satisfies with sdk's `facebookReactNativeVersion`
    ...
    if (!_semver().default.satisfies(reactNative, `~${facebookReactNativeVersion}`)) {
      _internal2().ProjectUtils.logWarning(projectRoot, 'expo', `Warning: Invalid version of react-native for sdkVersion ${sdkVersion}. Use react-native@${facebookReactNativeVersion}`, 'doctor-invalid-version-of-react-native');
